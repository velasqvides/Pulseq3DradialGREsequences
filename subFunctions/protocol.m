classdef protocol < handle
    ...
    methods
        ...
        function dwellTime = get.dwellTime(obj)            
           dwellTime =  obj.ADCrasterTime * round( 1 / (obj.nSamples * obj.readoutOversampling * obj.bandwidthPerPixel) / obj.ADCrasterTime );
        end
        
        function readoutGradientFlatTime = get.readoutGradientFlatTime(obj)            
            % due to gradient raster time constraints, the flat area is not
            % the same as the readout duration, but it has to be a bit larger.
