    ...
    methods
        ...
        function validateResolutionZ(obj)
            ...
            if obj.partitionThickness < obj.partitionThickness_min
                ...
                toPrintPartitions = sprintf('**nPartitions = %i has to be changed to %i to keep the partition thickness above the lower limit: %2.1f mm\n',nPartitions_old, br, obj.partitionThickness_min*1e3);
                obj.nPartitions = br;
            elseif obj.partitionThickness > obj.partitionThickness_max
                br = ceil(obj.slabThickness / obj.partitionThickness_max);
                toPrintPartitions = sprintf('**nPartitions = %i has to be changed to %i to keep the partition thickness below the upper limit: %2.1f mm\n',nPartitions_old, br, obj.partitionThickness_max*1e3);
                obj.nPartitions = br;            
            end
            ...
            if obj.RfPulseDuration < obj.RfPulseDuration_min
                toPrintRf = sprintf('**RF pulse duration = %3.1f us is below lower limit %3.1f us. RfPulseDuration set to: %3.1f us\n',RfPulseDuration_old*1e6, obj.RfPulseDuration_min*1e6, obj.RfPulseDuration_min*1e6);
            ...
            if obj.transmitterBandwidth < obj.transmitterBandwidth_min
                TBW_new = 2 * ceil(obj.transmitterBandwidth_min * obj.RfPulseDuration / 2); % to be sure timeBwProduct is multiple of 2
            ...
            elseif obj.transmitterBandwidth > obj.transmitterBandwidth_max
                TBW_new = 2 * floor(obj.transmitterBandwidth_max * obj.RfPulseDuration / 2); % to be sure timeBwProduct is multiple of 2
                text1= sprintf('**timeBwProduct = %i was changed to %i',obj.timeBwProduct, TBW_new);
            ...
            if obj.slabGradientAmplitude > obj.systemLimits.maxGrad % maxGrad in Hertz
                ...
                text1 = sprintf('**RfPulseDuration = %3.1f us was changed to %3.1f us',RfPulseDuration_old*1e6, RFduration_new*1e6);
        ...
        function validateTEandTR(obj)
            ...
            assert(isTR_maxCorrect,'The constant property TR_max in Class Protocol seems to have changed, make sure this new value is multiple of (%.2f ms)\n',gradRasterTime*1e3);
