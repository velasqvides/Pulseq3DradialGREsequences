...
report{end+1} = sprintf('%.3f <= TE <= %.3f\n',TE_min*1e3,TE_max*1e3);
report{end+1} = sprintf('%.3f <= TR <= %.3f, ',TR_min*1e3,obj.TR_max*1e3);
report{end+1} = sprintf('with the constraint: TR >= TE +%.3f\n',(TR_min*1e3 - TE_min*1e3));

% Check or fix TE first
if obj.TE + 2 * eps < TE_min
    report{end+1} = sprintf(['**Current TE =%.3f ms is below lower limit %.3f ms,'...
        ' TE set to:%.3f ms\n'],obj.TE*1e3, TE_min*1e3, TE_min*1e3);
    obj.TE = TE_min;
elseif obj.TE > TE_max + 2 * eps
    report{end+1} = sprintf(['**Current TE =%.3f ms is above upper limit %.3f ms,'...
...
if obj.TR > obj.TR_max + 2 * eps
    report{end+1} = sprintf(['**Current TR =%.3f ms is above upper limit %.3f ms,'...
...
elseif obj.TR + 2 * eps < obj.TE + (TR_min - TE_min)
    report{end+1} = sprintf(['**The time difference between any valid TE and TR must'...
        ' be at least:%.3f ms. TR set to:%.3f ms\n'],...
